...
/// Range: approximately -32768.0 to 32767.99998 with precision of 1/65536.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
pub struct Fixed(i32);

impl Fixed {
    /// The value representing 1.0 in fixed-point.
    pub const ONE: Fixed = Fixed(1 << 16);
    /// The value representing 0.0 in fixed-point.
    pub const ZERO: Fixed = Fixed(0);
    /// The fractional bits (16).
    pub const FRAC_BITS: u32 = 16;

    /// Creates a Fixed from an integer value.
    #[inline]
    pub const fn from_int(n: i32) -> Self {
        Self(n << Self::FRAC_BITS)
    }

    /// Creates a Fixed from a raw i32 value (already in Q16.16 format).
    #[inline]
    ...
    /// Converts to an integer (truncates fractional part).
    #[inline]
    pub const fn to_int(self) -> i32 {
        self.0 >> Self::FRAC_BITS
    }

    /// Converts from f32 (for authoring layer, NOT for runtime).
    /// Use only for loading from editor formats; runtime uses raw values.
    #[inline]
    pub fn from_f32(f: f32) -> Self {
        Self((f * (1 << Self::FRAC_BITS) as f32) as i32)
    }

    /// Converts to f32 (for display purposes, NOT for interpolation).
    #[inline]
...
impl std::ops::Div for Fixed {
    ...
    fn div(self, rhs: Self) -> Self {
        if rhs.0 == 0 {
            return Self::ZERO; // Avoid division by zero
        }
...
impl Easing {
    ...
    pub fn apply(self, t: Fixed) -> Fixed {
        match self {
            ...
            Easing::EaseInOut => {
                // Piecewise: 2t^2 for t < 0.5, 1 - 2(1-t)^2 for t >= 0.5
...
pub enum PropertyType {
    ...
    /// Z-order (layer).
    ZOrder,
    /// Scale (fixed-point, 1000 = 1.0x).
    Scale,
    /// Opacity (0-1000).
    Opacity,
    /// Rotation in degrees (fixed-point).
    Rotation,
}

/// A value for an animatable property.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
///
/// # Contract
/// - `time` is measured in ticks (not seconds) for determinism.
/// - Keyframes in a track must have strictly increasing times.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Keyframe {
    /// Time in ticks when this keyframe occurs.
    pub time: u32,
...
mod tests {
    ...
    fn test_timeline_evaluate() {
        ...
        assert!(values.iter().any(|(id, prop, val)| *id == entity
            && *prop == PropertyType::PositionX
            && *val == 50));
        assert!(values.iter().any(|(id, prop, val)| *id == entity
            && *prop == PropertyType::PositionY
            && *val == 100));
    }
