...
fn sanitize_relative_path(path: &Path, field_name: &str) -> VnResult<PathBuf> {
...
fn canonicalize_within_root(root: &Path, path: &Path, field_name: &str) -> VnResult<PathBuf> {
...
fn invalid_bundle(message: impl Into<String>) -> VnError {
