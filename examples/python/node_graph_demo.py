...
def create_simple_story():
    ...
    dialogue1 = graph.add_node(
        StoryNode.dialogue("Alice", "Welcome to the enchanted forest!"),
        0, 200
    )
    dialogue2 = graph.add_node(
        StoryNode.dialogue("Bob", "It's beautiful here."),
        0, 300
    )
...
def create_branching_story():
    ...
    intro = graph.add_node(
        StoryNode.dialogue("Narrator", "You find yourself at a crossroads."),
        0, 100
    )
    
    # Choice
    choice = graph.add_node(
        StoryNode.choice("Which path do you take?", [
            "Go left into the forest",
            "Go right to the mountains"
        ]),
        0, 200
    )
    ...
    left_dialogue = graph.add_node(
        StoryNode.dialogue("Forest Spirit", "Welcome, traveler."),
        -150, 400
    )
    ...
    right_dialogue = graph.add_node(
        StoryNode.dialogue("Mountain Guide", "The peak awaits!"),
        150, 400
    )
...
def validate_story(graph):
    ...
    # Check for errors
    has_errors = any(
        str(issue.severity) == "LintSeverity.Error" 
        for issue in issues
    )
...
def create_invalid_story():
    ...
    graph.add_node(
        StoryNode.dialogue("Someone", "Hello!"),
        0, 0
    )
